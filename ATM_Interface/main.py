...
from database import ATM_Manager as ATM_DB
...
class ATM_Login(ttk.Window):
    def __init__(self, title, size):
        ...
        self.userID_entry = ttk.Entry(self, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                      textvariable=self.userID_var)
        ...
        self.password_entry = ttk.Entry(self, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center", show="*",
                                        textvariable=self.userpassword_var)
        ...
        self.login_button = ttk.Button(self.button_frame, text="Login",
                                       command=lambda: self.login_function(self.userID_var.get(),
                                                                           self.userpassword_var.get()))
    ...
    def login_function(self, user_id, password):
        ...
        if self.acc_no[0]:
            ...
            self.iconify()
            ATM_GUI("ATM", (820, 600), self.acc_no)
        else:
            msg.showerror("Login Failed", self.acc_no[1])

    def register_function(self):
        self.iconify()
        ATM_Register("Register", (800, 800))
...
class ATM_Register(ttk.Toplevel):
    def __init__(self, title, size):
        ...
        self.surname_entry = ttk.Entry(user_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                       textvariable=self.surname_var)
        self.surname_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # First Name Label
        self.fstname_label = ttk.Label(user_info_frame, text="First Name", font=("Arial", 20, 'bold'),
                                       bootstyle='primary')
        self.fstname_label.pack(side="top", padx=20, pady=10, anchor='center')

        # First Name Entry
        self.fstname_var = tk.StringVar()
        self.fstname_entry = ttk.Entry(user_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                       textvariable=self.fstname_var)
        self.fstname_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # Home Address Label
        self.homeadd_label = ttk.Label(user_info_frame, text="Home Address", font=("Arial", 20, 'bold'),
                                       bootstyle='primary')
        self.homeadd_label.pack(side="top", padx=20, pady=10, anchor='center')

        # Home Address Entry
        self.homeadd_var = tk.StringVar()
        self.homeadd_entry = ttk.Entry(user_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                       textvariable=self.homeadd_var)
        self.homeadd_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # Phone Number Label
        self.phonenum_label = ttk.Label(user_info_frame, text="Phone Number", font=("Arial", 20, 'bold'),
                                        bootstyle='primary')
        self.phonenum_label.pack(side="top", padx=20, pady=10, anchor='center')

        # Phone Number Entry
        self.phonenum_var = tk.StringVar()
        self.phonenum_entry = ttk.Entry(user_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                        textvariable=self.phonenum_var)
        self.phonenum_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # UserID Label
        self.userID_label = ttk.Label(account_info_frame, text="User ID", font=("Arial", 20, 'bold'),
                                      bootstyle='primary')
        self.userID_label.pack(side="top", padx=20, pady=10, anchor='center')

        # UserID Entry
        self.userID_var = tk.StringVar()
        self.userID_entry = ttk.Entry(account_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                      textvariable=self.userID_var)
        self.userID_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # Password Label
        self.password_label = ttk.Label(account_info_frame, text="Password", font=("Arial", 20, 'bold'),
                                        bootstyle='primary')
        self.password_label.pack(side="top", padx=20, pady=10, anchor='center')

        # Password Entry
        self.userpassword_var = tk.StringVar()
        self.password_entry = ttk.Entry(account_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                        show="*", textvariable=self.userpassword_var)
        self.password_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # Initial Deposit Label
        self.initdep_label = ttk.Label(account_info_frame, text="Initial Deposit", font=("Arial", 20, 'bold'),
                                       bootstyle='primary')
        self.initdep_label.pack(side="top", padx=20, pady=10, anchor='center')

        # Initial Deposit Entry
        self.initdep_var = tk.StringVar()
        self.initdep_entry = ttk.Entry(account_info_frame, font=("Berlin Sans FB Demi", 15, 'bold'), justify="center",
                                       textvariable=self.initdep_var)
        self.initdep_entry.pack(side="top", padx=20, pady=10, anchor='center')

        # Submit Label
        self.submit_label = ttk.Label(account_info_frame, text="Submit?", font=("Arial", 20, 'bold'),
                                      bootstyle='primary')
    ...
    def submit_func(self):
        ...
        if (self.surname_var.get() == "" or self.fstname_var.get() == "" or self.homeadd_var.get() == "" or
                self.phonenum_var.get() == "" or self.userID_var.get() == "" or self.userpassword_var.get() == "" or
                self.initdep_var.get() == ""):
            ...
            if self.atm_db.view_user(self.userID_var.get()) != None:
                ...
                # Create user
                self.atm_db.register_users(self.userID_var.get(), self.surname_var.get(), self.fstname_var.get(),
                                           self.homeadd_var.get(), self.phonenum_var.get())
                # Create account
...
class infoFrame(ttk.Frame):
    def __init__(self, master):
        ...
        self.refresh_button = ttk.Button(self, text="Refresh", command=self.refresh).pack(side="top", padx=20, pady=10,
                                                                                          anchor='w')
    ...
    def create_balance(self):
        ...
        self.balance_label = ttk.Label(balance_frame, text=self.balance_var, font=("Berlin Sans FB Demi", 20, 'bold'),
...
class Deposit_Window(ttk.Toplevel):
    ...
    def submit_func(self, account_number, amount):
        ...
        msg.showinfo("Deposit Successful",
...
class Withdraw_Window(ttk.Toplevel):
    ...
    def submit_func(self, account_number, amount):
        ...
        if type(status) == tuple:
            msg.showinfo("Withdrawal Successful",
                         f"Your new balance is:\n\u20b1{self.master.atm_db.view_balance(account_number)}.\n{status[1]}")
        else:
...
class Transactions_Window(ttk.Toplevel):
    def __init__(self, master):
        ...
        for transaction in self.master.atm_db.view_all_transactions(self.master.acc_no):
            self.table.insert(parent='', index='end', iid=id, text="",
...
if __name__ == "__main__":
    ATM_Login("Login to your account", (500, 580))
